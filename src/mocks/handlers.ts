...
function page<T>(items: T[], number: number, size: number): PageResponse<T> {
...
export const authHandlers = [
  http.post(`${api}/auth/login`, async ({ request }) => {
    ...
    return body.email === mockCredentials.email && body.password === mockCredentials.password
      ? HttpResponse.json(createMockSession())
      : HttpResponse.json(
          { message: 'E-posta veya şifre hatalı.', status: 401, correlationId: 'mock-auth-error' },
          { status: 401 },
        );
  }),
];
...
export const dataHandlers = [
    ...
    return HttpResponse.json(
      mockCalls.filter(
        (item) =>
          ...
          (!q ||
            item.roomName.toLocaleLowerCase('tr-TR').includes(q) ||
  ...
  http.get(`${api}/calls/:id`, async ({ params }) => {
    await delay(120);
    const item = mockCalls.find((call) => call.id === params.id);
    return item
      ? HttpResponse.json(item)
      : HttpResponse.json({ message: 'Çağrı bulunamadı.' }, { status: 404 });
  }),
  http.get(`${api}/appointments`, async ({ request }) => {
    ...
    const items = mockAppointments.filter(
      (item) =>
        (!storeId || item.storeId === storeId) &&
        (!status || item.status === status) &&
        (!from || new Date(item.startTime) >= new Date(from)) &&
        (!to || new Date(item.startTime) <= new Date(to)),
    );
    return HttpResponse.json(page(items, number, size));
  }),
  http.get(`${api}/appointments/:id`, async ({ params }) => {
    await delay(100);
    const item = mockAppointments.find((appointment) => appointment.id === params.id);
    return item
      ? HttpResponse.json(item)
      : HttpResponse.json({ message: 'Randevu bulunamadı.' }, { status: 404 });
  }),

...
export function useCallRows(calls: Call[]) {
  const appointmentQueries = useQueries({
    queries: calls.map((call) => ({
      queryKey: queryKeys.medicalAppointments.detail(call.appointmentId),
      queryFn: () => getMedicalAppointment(call.appointmentId),
      staleTime: 30_000,
      retry: false,
    })),
  });
  return {
    rows: calls.map((call, index) => toCallRow(call, appointmentQueries[index]?.data)),
    isLoadingNames: appointmentQueries.some((query) => query.isLoading),
  };
}

...
export function useCalls(filters: CallsFilters) {
  ...
  return useQuery({ queryKey: queryKeys.calls.list(filters), queryFn: async () => { const data = await getCalls(filters); touch(); return data; }, refetchInterval: mode === 'live' ? 30_000 : 5_000, refetchIntervalInBackground: false });
}

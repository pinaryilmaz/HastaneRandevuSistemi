...
export function LoginPage() {
  ...
  return (
    <main className="min-h-screen bg-navy-900 lg:grid lg:grid-cols-[1.08fr_0.92fr]">
      <section className="relative hidden overflow-hidden p-12 text-white lg:flex lg:flex-col lg:justify-between">
        ...
        <div className="relative flex items-center gap-3">
          ...
          <div>
            <p className="font-semibold">Hastane Operasyon Merkezi</p>
            <p className="text-xs text-slate-300">Agentic AI destekli randevu yönetimi</p>
          </div>
        </div>
        <div className="relative max-w-xl">
          <p className="mb-4 text-sm font-semibold uppercase tracking-[0.2em] text-aqua-100">
            Canlı kontrol kulesi
          </p>
          <h1 className="text-5xl font-semibold leading-tight">
            Her çağrı, her randevu, tek güvenli görünümde.
          </h1>
          <p className="mt-6 max-w-lg text-lg leading-8 text-slate-300">
            Yapay zekâ destekli çağrı akışlarını, hasta randevularını ve servis sağlığını gerçek
            zamanlı izleyin.
          </p>
          <div className="mt-10 grid grid-cols-3 gap-3">
            {[
              [Activity, 'Anlık metrikler'],
              [Radio, 'Canlı olaylar'],
              [CheckCircle2, 'Uçtan uca izleme'],
            ].map(([Icon, text]) => {
        ...
        <p className="relative text-xs text-slate-400">
          Kişisel sağlık verileri güvenli ve maskeli gösterilir.
        </p>
      </section>
      <section className="flex min-h-screen min-w-0 items-center justify-center overflow-x-hidden bg-slate-50 px-4 py-8 sm:px-5 sm:py-10">
        <div className="w-[calc(100vw-2rem)] min-w-0 max-w-md sm:w-full">
          <div className="mb-8 lg:hidden">
            ...
            <p className="mt-2 font-semibold text-navy-900">Hastane Operasyon Merkezi</p>
          </div>
          <div className="min-w-0 max-w-full rounded-3xl border border-slate-200 bg-white p-5 shadow-panel sm:p-9">
            <p className="text-sm font-semibold text-aqua-700">Operasyon paneli</p>
            ...
            <p className="mb-8 mt-2 text-sm leading-6 text-slate-500">
              Yetkili kurum hesabınızla devam edin.
            </p>
            <LoginForm />
            <div className="mt-6 break-words rounded-xl bg-slate-50 p-3 text-xs leading-5 text-slate-500">
              <strong>{env.useMockAuth ? 'Demo' : 'Yerel backend'}:</strong> Formdaki örnek operatör
              hesabıyla giriş yapabilirsiniz.
            </div>
          </div>
        </div>
      </section>
